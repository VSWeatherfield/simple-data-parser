...
struct ValidatedReportParserTests : testing::Test {
    ...
    const std::string validReport = to_string(
        json{{"payer", 1}, {"tax", taxName}, {"amount", 1000}, {"year", 2020}});

    const std::vector<std::string> invalidReports = {
        to_string(json{
            {"payer", 1}, {"tax", taxName}, {"amount", 0.0}, {"year", 2020}}),
        to_string(json{
            {"payer", 1}, {"tax", taxName}, {"amount", 1000}, {"year", 2222}}),
...
TEST_F(ValidatedReportParserTests, reportValidationSucceeds) {
    ASSERT_NE(sut.parse(validReport), std::nullopt);
}
