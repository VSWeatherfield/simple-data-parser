...
ReportSession::ReportSession(
    tcp::socket sock, const auth::IAuthentication& authManager,
...
void ReportSession::start() try {
    if (const auto user = loginUser()) {
        boost::asio::write(sock, boost::asio::buffer(constants::OK.data(),
                                                     constants::OK.size()));
        ...
        boost::asio::write(sock, boost::asio::buffer(constants::NOK.data(),
                                                     constants::NOK.size()));
        stop();
