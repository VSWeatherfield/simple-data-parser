...
bool AuthManager::authenticate(const types::Login& login,
                               const types::Password& password) const {
    const auto it = users.find(login);
    return it != users.end() ? it->password.value == password.value : false;
}

bool AuthManager::isAuthorized(const types::Login& login,
                               std::uint32_t taxPayerId) const {
    const auto it = users.find(login);
