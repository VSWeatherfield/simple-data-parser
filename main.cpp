...
constexpr std::array<const char*, 4> EXPECTED_FIELDS = {"payer", "tax",
                                                        "amount", "year"};
...
class ReportingSession {
   ...
   public:
    ReportingSession(tcp::socket sock, ReportFormat format)
        : sock{std::move(sock)}, format{format} {}
   ...
   private:
    ...
    std::string handleJsonRequest(const std::string_view request) {
        ...
        if (json.empty()) return "NOK";

        for (const auto& field : EXPECTED_FIELDS)
            if (json[field].empty()) return "NOK";
    ...
    std::string handleXmlRequest(const std::string_view request) {
        ...
        if (root == nullptr) return "NOK";

        for (const auto& field : EXPECTED_FIELDS)
            if (root->FirstChildElement(field) == nullptr) return "NOK";

        return "OK";
    }
};

void server(boost::asio::io_context& io_context, std::uint16_t port,
            ReportFormat format) {
    tcp::acceptor a(io_context, tcp::endpoint(tcp::v4(), port));
    for (;;) {
        std::thread(&ReportingSession::start,
                    ReportingSession(a.accept(), format))
            .detach();
    }
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        const auto format = std::strcmp(argv[2], "json") == 0
                                ? ReportFormat::Json
                                : ReportFormat::Xml;
